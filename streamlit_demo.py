...
if page == "Trash Classification":
    ...
    st.markdown("<p style='text-align:center; color:#636e72; font-size: 1.1rem;'>Upload, paste a link, or capture live to classify waste with AI.</p>", unsafe_allow_html=True)

    option = st.radio("Choose Input Method:", ["Upload Image", "Paste Image Link", "Live Camera Capture"])
    ...
    if option == "Upload Image":
        ...
        if uploaded_file:
            ...
            st.image(image_data, caption="Uploaded Trash Image", use_container_width=True)

    # --- Paste Image Link ---
    elif option == "Paste Image Link":
        ...
        if img_url:
            try:
                ...
                if response.status_code == 200:
                    ...
                    st.image(image_data, caption="Image from Link", use_container_width=True)
                else:
                    st.error("Could not fetch image from link.")
            except Exception as e:
                st.error(f"Error fetching image: {e}")

    # --- Live Camera Capture ---
    elif option == "Live Camera Capture":
        camera_input = st.camera_input("Take a picture")
        if camera_input:
            image_data = camera_input.getvalue()
            st.image(image_data, caption="Live Captured Image", use_container_width=True)

    # --- Model Selection ---
    model_choice = st.selectbox("Choose Model", ["latest"], index=0)

    # --- Classification Button ---
    if image_data and st.button("Classify Trash"):
        with st.spinner('AI is classifying the trash...'):
            try:
                # Send image + model choice to Flask backend
                files = {"file": image_data}
                data = {"model": model_choice}
                response = requests.post("http://127.0.0.1:5000/classify", files=files, data=data)
                ...
                if response.status_code == 200:
                    ...
                    # --- Dictionary mapping for contextual messages ---
                    classification_messages = {
                        "cardboard": ("This item is recyclable! ♻️", "info"),
                        "paper": ("This item is recyclable! ♻️", "info"),
                        "metal": ("This material can be recycled efficiently. 🔄", "info"),
                        "glass": ("This material can be recycled efficiently. 🔄", "info"),
                        "plastic": ("Plastic detected. Consider recycling responsibly. 🚯", "warning"),
                        "trash": ("This is general waste. 🚮", "error"),
                    }

                    message, msg_type = classification_messages.get(
                        predicted_class,
                        ("Could not determine recycling instructions.", "warning")
                    )
                    ...
                    st.markdown(f"""
                        <div class="result-card">
                            ...
                            <p style="font-size: 1.2rem; color: #adb5bd;">
                                <strong>Model:</strong> <span style="color: #89d957;">{model_used}</span><br>
                                <strong>Class:</strong> <span style="color: #89d957; font-weight: 700;">{predicted_class.upper()}</span>
                            </p>
                            {"<p style='color:#adb5bd;'>Confidence: {:.2f}</p>".format(confidence) if confidence else ""}
                        </div>
                    """, unsafe_allow_html=True)

                    # --- Show contextual recycling tip ---
                    if msg_type == "info":
                        st.info(message)
                ...
                st.error("⚠️ Flask server not running.")
            except Exception as e:
...
if page == "Bin Fill Prediction":
    st.subheader("Predicted Bin Fill Levels (Next 24 Hours)")
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            if predictions:
                df = pd.DataFrame({
                    "Hour": list(range(240, 240 + len(predictions))),
                    "Predicted Fill Level (%)": predictions
                })

                st.line_chart(df.set_index("Hour"))
                st.success("✅ Predictions retrieved successfully from Flask backend")

                latest_pred = predictions[-1]
                st.markdown(f"""
                    <div class="result-card">
                        <h3 style="color: #f8f9fa;">Latest Prediction</h3>
                        <p style="font-size: 1.2rem; color: #adb5bd;">
                            <strong>Bin fill after 24 hours:</strong>
                            <span style="color: #89d957; font-weight: 700;">{latest_pred:.1f}%</span>
                        </p>
                    </div>
                """, unsafe_allow_html=True)
            else:
                st.warning("No predictions available from Flask API.")
        else:
            st.error(f"Error {response.status_code}: {response.text}")
    except requests.exceptions.ConnectionError:
        st.error("⚠️ Could not connect to Flask backend. Make sure `app1.py` is running.")
